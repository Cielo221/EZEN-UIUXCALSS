...
// onreadystatechange : 클라이언트가서버에 데이터를 요청하고 서버에서 데이터를 전송해주는 과정속에서 데이터가 이동될 때마다 실행되는 이벤트 핸들러
...
// 4 : 서버가 정상적으로 클라이언트측에 데이터를 전송완료했고, 이제 클라이언트츧에서 데이터를 마음껏 사용할 수 있는 상태

xhr.onreadystatechange = function () {
  if (xhr.readyState == 4 && xhr.status == 200) {
    const student = JSON.parse(xhr.responseText);
    document.write(`${student.name} 학생은 ${student.grade}입니다.`);
  }
};
